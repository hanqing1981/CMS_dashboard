...
class Get_put_post():
    ...
    def get(self,subject,payload=None,quering=None): #quering: {filter:392609912},
    ...
    def apiConnTest(self):
        try:
            self.url=self.baseurl+'system/alarms'
            self.response = requests.get(self.url, verify=False,
                                        auth=HTTPBasicAuth(self.username, self.password), stream=True,timeout=2)
            if self.response.status_code==200:
                # print(self.response.status_code,'****')
                return (etree.HTML(self.response.text))
            else:
                # print(self.response.status_code,'****')
                print('failed to connect to API port %s' %self.response.status_code)
                return (0)
        except Exception as error:
            print(error)
            return (0)
